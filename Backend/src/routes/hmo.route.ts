...
const hmoRoute = factory
  ...
  .post("/", zValidator("json", createHmoSchema), async (c) => {
    try {
      ...
      // coverageStates and planTypes are already JSON strings from frontend

      const newHmo = await db.transaction(async (tx) => {
        const [hmo] = await tx.insert(hmos).values(payload).returning();
        return hmo;
      });

      return c.json({ success: true, data: newHmo });
    } catch (error: any) {
      console.error("[HMO] Creation Error:", error);
      return c.json({ 
        success: false, 
        error: error.message || "Failed to create HMO",
